...
// Global timer source object
static btstack_timer_source_t hid_timer;

// Timer handler function
static void my_timer_handler(btstack_timer_source_t * timer){
    // Re-arm the timer to fire again in TIMER_INTERVAL_MS ms
    btstack_run_loop_set_timer(timer, TIMER_INTERVAL_MS);
    btstack_run_loop_add_timer(timer);
    bluetooth_send_report();
}

// Somewhere in your init code, setup and start the timer
void start_my_repeating_timer(void){

    // Set timer delay
    btstack_run_loop_set_timer_handler(&hid_timer, &my_timer_handler);
    btstack_run_loop_set_timer(&hid_timer, TIMER_INTERVAL_MS);
